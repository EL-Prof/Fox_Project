...
public class SearchRepair extends javax.swing.JFrame {
    ...
    private int returnRowIndexForValue(int column , String value) 
    { 
      for (int i = 0; i < searchReprTbl.getRowCount(); i++)
      {
                if (searchReprTbl.getValueAt(i, column).equals(value))
                {
                    return i ;
                }
    ...
    private boolean ValueIsExist(int column , String value) 
    { 
      for (int i = 0; i < searchReprTbl.getRowCount(); i++)
      {
                if (searchReprTbl.getValueAt(i, column).equals(value))
    ...
    private void searchReprBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchReprBtnActionPerformed
        if(!ValueIsExist(8, searchReprTxt.getText()))
        {
    ...
    private void reprConfirmBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_reprConfirmBtnActionPerformed
        if (searchReprTbl.getValueAt(returnRowIndexForValue(8, reprConfirmCbox.getSelectedItem().toString()), 4).equals("")) {
            String timeStamp = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(Calendar.getInstance().getTime());
            String query = "UPDATE repair SET reset_data = '" + timeStamp + "', paid = '" + searchReprTbl.getValueAt(returnRowIndexForValue(8, reprConfirmCbox.getSelectedItem().toString()), 3) + "' where code = '" + reprConfirmCbox.getSelectedItem().toString() + "'";
            try {
                DB.stmt.executeUpdate(query);
                searchReprTbl.setValueAt(timeStamp, returnRowIndexForValue(8, reprConfirmCbox.getSelectedItem().toString()), 4);
                searchReprTbl.setValueAt(searchReprTbl.getValueAt(returnRowIndexForValue(8, reprConfirmCbox.getSelectedItem().toString()), 3), returnRowIndexForValue(8, reprConfirmCbox.getSelectedItem().toString()), 2);
                searchReprTbl.setValueAt(0.0, returnRowIndexForValue(8, reprConfirmCbox.getSelectedItem().toString()), 1);
            } catch (SQLException ex) {
                Logger.getLogger(DB.class.getName()).log(Level.SEVERE, null, ex);
                System.out.println("error " + ex.getMessage());
            }
